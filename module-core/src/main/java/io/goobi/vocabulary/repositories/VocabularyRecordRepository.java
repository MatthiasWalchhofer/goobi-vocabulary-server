...
public interface VocabularyRecordRepository extends ListCrudRepository<VocabularyRecordEntity, Long> {
    ...
    Page<VocabularyRecordEntity> findByVocabulary_IdAndParentRecordNullAndMetadataFalse(@NonNull long id, Pageable pageable);
     ...
     */
    @Query(value = "SELECT r1.id,r1.parent_record_id,r1.vocabulary_id,r1.is_metadata FROM (" +
            ...
            "WHERE r.vocabulary_id=?1 AND f.field_definition_id=?2 and r.parent_record_id IS NULL and r.is_metadata IS false " +
    ...
    @Query(value = "SELECT DISTINCT vre1_0.id,vre1_0.parent_record_id,vre1_0.vocabulary_id,vre1_0.is_metadata FROM vocabulary_record vre1_0 " +
            ...
            "where vre1_0.vocabulary_id=?1 and t1_0.content like ?2 escape '\\\\' and vre1_0.is_metadata IS false", nativeQuery = true)
    ...
    @Query(value = "SELECT r1.id,r1.parent_record_id,r1.vocabulary_id,r1.is_metadata FROM (" +
            ...
            "LEFT JOIN field_value v ON f.id=v.field_instance_id " +
            "LEFT JOIN field_translation t ON v.id=t.field_value_id " +
            "WHERE r.vocabulary_id=?1 AND t.content like ?2 escape '\\\\' AND f.field_definition_id=?3 and r.is_metadata IS false " +
            "ORDER BY t.content ASC" +
            ") AS sorted_records " +
            "LEFT JOIN vocabulary_record r1 ON sorted_records.id=r1.id", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT vre1_0.id,vre1_0.parent_record_id,vre1_0.vocabulary_id,vre1_0.is_metadata FROM vocabulary_record vre1_0 " +
            ...
            "left join field_value fv1_0 on f1_0.id=fv1_0.field_instance_id " +
            "left join field_translation t1_0 on fv1_0.id=t1_0.field_value_id " +
            "where vre1_0.vocabulary_id=?1 and f1_0.field_definition_id=?2 and t1_0.content like ?3 escape '\\\\' and vre1_0.is_metadata IS false", nativeQuery = true)
